...
export interface BlueprintOption {
  ...
  emoji: string
  color: string
  steps: Array<{ title: string; description: string; skillId: string | null }>
}
...
function blankSteps(): FormStep[] {
  return [
    ...
    { id: `s-${Date.now()}-2`, title: '', description: '', skillId: null },
    { id: `s-${Date.now()}-3`, title: '', description: '', skillId: null },
  ]
}
...
export function CreateProjectForm({
  ...
  const addStep = () => {
    setSteps((prev) => [
      ...prev,
      { id: `s-${Date.now()}-${prev.length}`, title: '', description: '', skillId: null },
    ])
  }
    ...
    <div className="grid grid-cols-[28px_1fr_32px] items-start gap-3 rounded-xl border border-white/[0.08] bg-bg-base p-4 transition-colors duration-fast hover:border-neutral-700 focus-within:border-amber-500 sm:grid-cols-[28px_1fr_200px_32px]">
      ...
      <div ref={skillRef} className="relative col-start-2 row-start-2 sm:col-start-3 sm:row-start-1">
        ...
        >
          ...
          <ChevronDown className={cn('size-3 shrink-0 transition-transform', skillOpen && 'rotate-180')} />
        </button>
        {skillOpen && (
